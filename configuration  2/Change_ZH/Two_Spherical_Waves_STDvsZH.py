...
# -*- coding: utf-8 -*-
"""
Created on Mon Dec  5 12:23:35 2022
...
# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 20:34:03 2022
...
class hologram_simulation(object):
    ...
    def __init__(self):
        ...
        self.z_h = 200     # Distance between camera and concave mirror (mm)
        #self.z_h = np.arange(80,100,4)  
        #self.z_h = np.append( np.arange(220,240,step_size), np.arange(240,260,step_size)+step_size ) 
        self.f_slm1 = 300
        self.f_slm2 = 700
        #self.mag = 50
        ...
        self.z_depth = np.arange((z_s-0.003), (z_s+0.003), self.dz)        #recon range(z_depth(-5um to 5um, step size:100 nm))
    ...
    def Get_holo_radius_realsetting(self, z_s, z_h):
        Holo_R_stack1 = self.Hologram_radius_realsetting(z_s, z_h, self.f_slm1, self.f_o, self.na)
        Holo_R_stack2 = self.Hologram_radius_realsetting(z_s, z_h, self.f_slm2, self.f_o, self.na)
        Holo_R_stack_real = min(np.abs(Holo_R_stack1), np.abs(Holo_R_stack2))
        return Holo_R_stack_real
    
    
    # def Get_ZH_at_perfect_overlap_realsetting(self, z_s):
    #     if self.f_slm1 > self.f_slm2:
    #         z_h_range = np.arange(self.f_slm2,self.f_slm1,1)
    #     elif self.f_slm1 < self.f_slm2:
    #         z_h_range = np.arange(self.f_slm1,self.f_slm2,1)
    ...
    # def I_function(self, sx ,sy, x, y, wl, recon_dist, theta):
    #     I = 2 + np.exp(((1j * np.pi / (wl * recon_dist) ) * (2 * sx * x + 2 * sy * y + x**2 + y**2)) + (1j * theta)) + np.exp(((-1j * np.pi / (wl * recon_dist) ) * (2 * sx * x + 2 * sy * y + x**2 + y**2)) + (-1j * theta))
    #     return I.real
    ...
    def generate_holoImgs(self, z_s, z_h, recon_dist, sx, sy, verbose=False):
        ...
        self.Holo_R = self.Get_holo_radius_realsetting(z_s, z_h)
    ...
    def finch_recon3D(self, z_s, z_h, sx, sy):                                                         # 3D reconstruction of FINCH
        recon_dist = self.recon_dist_calc_realsetting(z_s, z_h)
        self.generate_holoImgs(z_s, z_h, recon_dist, 0, 0, False)
        #z_depth(-10um to 10um, step size:1 um)
        zr_depth = np.zeros(self.z_depth_n)
        for i in range(self.z_depth_n):
            zr_depth[i] = self.recon_dist_calc_realsetting(self.z_depth[i], z_h)                  #z_depth : 20um
        ...
        # tf.imshow(np.abs(self.Recon_3d_stack[50]))
        # 
    
    def get_STD(self, z_s, z_h, iteration, sx, sy):       #Calculate the STD of each Z_h(different radius)
        ...
        self.z_h = z_h
        Mt = self.Transverse_Magnification_realsetting(z_s,z_h)
        ### save images for Thunderstorm analysis
        #out_imgs = np.zeros((iteration, 2*self.nx, 2*self.ny))
        
        for i in range(iteration):
            #STD calculate for xy plane
            recon_dist = self.recon_dist_calc_realsetting(z_s, z_h)
            self.generate_holoImgs(z_s, z_h, recon_dist, sx, sy, True)
            self.finch_recon(recon_dist, True)
            Recon_xy = self.holo_recon
            "p=(amplitude, x_center, y_center, sigma_xy, background)"
            sec = 5             #size of sub_XY 
            ...
            self.finch_recon3D(self.z_s, z_h, sx, sy)          #self.Recon_3d_stack
            secz = 10              #zise of sub_Z
            ...
            init_params_zy = (1.e4, secz, sec, 1.5, 2., 0)    
    ...
    def Get_STD_vs_Z_h(self, z_s, z_h, iteration, sx, sy): #Calculate the STD of the hologram with different Z_h(different radius)
        try:
            os.mkdir(self.path)
        except:
            print('Directory already exists')
        self.z_h = z_h
        Recon_std_XY = np.zeros((2,len(self.z_h)))
        Recon_std_ZY = np.zeros((2,len(self.z_h)))
        
        Locs_mean_XY = np.zeros((2,len(self.z_h)))
        Locs_mean_ZY = np.zeros((2,len(self.z_h)))
        
        FWHM_XY = np.zeros((1,len(self.z_h)))
        FWHM_ZY = np.zeros((2,len(self.z_h)))
        
        Holo_R_stack = np.zeros((len(self.z_h)))
        for i in range(len(self.z_h)):
            self.fnd = 'z_h_%.2f' %z_h[i]
            Locs_xy, Locs_zy, Recon_std_xy, Recon_std_zy, FWHM_mean_xy, FWHM_mean_zy = self.get_STD(z_s,z_h[i], iteration, sx, sy)
        ...
        save_excel = np.zeros((10,len(z_h)))
        save_excel[0,:] = z_h
        ...
        df.index = ['Z_h', 'R_H', 'x_STD', 'y_STD','z_STD','x_Locs','y_Locs','z_Locs','xy_FWHM','z_FWHM']
        df.to_excel(self.path + '/' + 'TwoWaveZhChange'+str(self.bg) +'bg' + str(self.z_s) + 'z_s' + 'STD.xlsx', index=True)
        
        
        fig = plt.figure(figsize=(10,5))
        plt.plot(z_h,Holo_R_stack)
        plt.xlabel("z_h(mm)")
        plt.ylabel("R_h(mm)")
        plt.show()
        fig.savefig(self.path + '/' + 'Rh_vs_Zh.jpg',bbox_inches='tight',dpi=150)
        
        
        "plot all the results"
        fig = plt.figure(figsize=(10,5))
        plt.plot(z_h,Locs_mean_ZY[0,:])
        plt.xlabel("z_h(mm)")
        plt.ylabel("z_locs")
        plt.show()
        ...
        fig = plt.figure(figsize=(10,5))
        plt.plot(z_h,Locs_mean_XY[0,:])
        plt.plot(z_h,Locs_mean_XY[1,:])
        plt.legend(['x','y'])
        plt.xlabel("z_h(mm)")
        plt.ylabel("xy_locs")
        plt.show()
        ...
        fig = plt.figure(figsize=(10,5))
        plt.plot(z_h,Recon_std_XY[0,:])
        plt.plot(z_h,Recon_std_XY[1,:])
        plt.legend(['x','y'])
        plt.xlabel("z_h(mm)")
...
if __name__ == '__main__':
    ...
    z_h = np.append(np.arange(60,270,20),np.append(np.arange(330,670,20),np.arange(760,800,20)))
    t.Get_STD_vs_Z_h(z_s,z_h,iteration,sx,sy)
