...
class hologram_simulation(object):
    ...
    def get_STD(self, z_s, iteration, z_h, f_slm2, sx, sy):       #Calculate the STD of each Z_h(different radius)
        ...
        for i in range(iteration):
            ...
            self.zr_depth = self.finch_recon3D(z_s, z_h, f_slm2, sx, sy)          #self.Recon_3d_stack
    ...
    def Get_STD_vs_Z_s(self, z_s, iteration, z_h, f_slm2, sx, sy): #Calculate the STD of the hologram with different Z_h(different radius)
        try:
            os.mkdir(self.path)
        except:
        ...
        for i in range(len(z_s)):
            self.fnd = 'z_s_%.5f' %z_s[i]
            
            Locs_xy, Locs_zy, Recon_std_xy, Recon_std_zy, FWHM_mean_xy, FWHM_mean_zy = self.get_STD(z_s[i], iteration, z_h, f_slm2, sx, sy)
        ...
        df.to_excel(self.path + '/' + str(self.bg) +'bg' + str(f_slm2) + 'f_slm2' + 'STD.xlsx', index=True)
        ...
        plt.ylabel("RH(mm))")
        plt.show()
        fig.savefig(self.path + '/' + 'RH_vs_Zs.jpg',bbox_inches='tight',dpi=150)
...
if __name__ == '__main__':
    ...
    t.Get_STD_vs_Z_s(z_s, iteration, z_h, f_slm2, sx, sy)
